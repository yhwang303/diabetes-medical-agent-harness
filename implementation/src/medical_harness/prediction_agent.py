"""Run-scoped prediction MCP tools. Only Core can execute and register numerical evidence."""
...
from .sdk_worker import MODEL, PREDICTION_AGENT_EXECUTOR, prediction_configuration
from .workers import WorkerLimits, WorkerRunner


class PredictionJobQuery(StrictModel):
    job_id: Annotated[str, Field(pattern=r'^[0-9a-f]{32}$')]


def binding(core, db, run):
    return digest({'configuration': prediction_configuration(), 'run_id': run['id'],
...
class PredictionTools:
    ...
    def validate(self, db):
        ...
        if job['step'] != 'prediction_agent' or job['input_digest'] != binding(self.core, db, run):
            raise GateError('PREDICTION_AGENT_BINDING')
        return run

    def inspect(self, db, run, job_id):
        job = db.execute('SELECT * FROM jobs WHERE id=? AND run_id=? AND step=?',
                         (job_id, run['id'], 'prediction')).fetchone()
        if not job:
        ...
        if not dependency or dependency['parent_job_id'] != self.agent_job_id:
            raise GateError('PREDICTION_JOB_BINDING')
        result = {'ok': True, 'job_id': job_id, 'status': job['status'],
                  'prediction_available': False, 'clinical_use': False}
        if job['status'] == 'ACCEPTED':
            body, artifact_hash = self.core._artifact(db, run, 'prediction')
            if body['job_id'] != job_id:
                raise GateError('PREDICTION_JOB_BINDING')
            result.update(prediction_available=True, evidence={
                'artifact_id': body['id'], 'artifact_hash': artifact_hash, 'origin': body['origin'],
                'producer': body['producer'], 'version': body['version']})
        elif job['status'] != 'RUNNING':
            result['reason'] = run['reason'] or 'PREDICTION_UNAVAILABLE'
        return result

    def call(self, name, raw):
        core = self.core
        try:
            if name == 'request_prediction':
                if raw != {}: raise GateError('INVALID_TOOL_ARGUMENTS', 422)
                with core.store.tx() as db:
                    run = self.validate(db)
                    old = db.execute("SELECT id FROM jobs WHERE run_id=? AND step='prediction' ORDER BY attempt DESC LIMIT 1",
                ...
                if result is None:
                    ...
                    core._model_step(self.owner, self.run_id, 'prediction', authorization_job_id=self.agent_job_id)
                    with core.store.tx() as db:
                        run = self.validate(db)
                        job = db.execute("SELECT id FROM jobs WHERE run_id=? AND step='prediction'", (self.run_id,)).fetchone()
                        if not job: raise GateError('PREDICTION_UNAVAILABLE')
                        result = self.inspect(db, run, job['id'])
            elif name == 'inspect_prediction':
                query = PredictionJobQuery.model_validate(raw)
                with core.store.tx() as db:
                    run = self.validate(db)
        ...
        with core.store.tx() as db:
            ...
            core._event(db, self.run_id, 'prediction_agent_tool', job_id=self.agent_job_id,
                        tool=name if name in ('request_prediction', 'inspect_prediction') else 'denied',
...
class PredictionAgent:
    ...
    def _receipt(self, db, run, agent_job_id):
        job = db.execute("SELECT id FROM jobs WHERE run_id=? AND step='prediction'", (run['id'],)).fetchone()
        if not job: raise GateError('PREDICTION_REQUIRED')
        result = PredictionTools(self.core, run['owner'], run['id'], agent_job_id).inspect(db, run, job['id'])
        if not result['prediction_available']: raise GateError(result.get('reason') or 'PREDICTION_REQUIRED')
        return dict(result, sdk_job_id=agent_job_id)

    def run(self, owner, run_id):
        core = self.core
        permission = core.data_permissions.authorize_run(owner, run_id, 'prediction')
        with core.store.tx() as db:
            run = core._run(db, run_id, owner)
            old = db.execute("SELECT * FROM jobs WHERE run_id=? AND step='prediction_agent'", (run_id,)).fetchone()
            if old:
                if old['status'] != 'ACCEPTED': raise GateError('PREDICTION_AGENT_ALREADY_STARTED')
                if old['input_digest'] != binding(core, db, run): raise GateError('PREDICTION_AGENT_BINDING')
                return self._receipt(db, run, old['id'])
            if run['state'] != 'SNAPSHOT_FROZEN': raise GateError('INVALID_STAGE')
        self.data_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
        with confined(self.data_dir / 'prediction.lock').open('a') as lock:
            ...
            with core.store.tx() as db:
                run = core._run(db, run_id, owner)
                core._live(db, run)
                if db.execute("SELECT 1 FROM jobs WHERE run_id=? AND step='prediction_agent'", (run_id,)).fetchone():
                    raise GateError('PREDICTION_AGENT_ALREADY_STARTED')
                if run['state'] != 'SNAPSHOT_FROZEN': raise GateError('INVALID_STAGE')
                agent_job_id = core._new_job(db, run, 'prediction_agent', binding(core, db, run))
                db.execute('UPDATE jobs SET deadline=? WHERE id=?',
                           (min(core.clock() + self.timeout, run['expires']), agent_job_id))
            self.evidence = {'run_id': run_id, 'job_id': agent_job_id, 'workers': [], 'clinical_use': False}
            tools = PredictionTools(core, owner, run_id, agent_job_id)
            def validate():
            ...
            try:
                ...
                with FlashGateway(key, self.data_dir / 'budget.json', None, transport=self.transport,
                    tool_handlers={name: (lambda args, name=name: tools.call(name, args))
                                   for name in ('request_prediction', 'inspect_prediction')}, authorize=validate) as gateway:
                    self.evidence['provider'] = gateway.records
                    with core.data_lifecycle.session(owner, run_id, agent_job_id) as session_id:
                        output = self.runner.run(PREDICTION_AGENT_EXECUTOR,
                            {'port': gateway.port, 'capability': gateway.capability, 'session': session_id,
                             'scenario': 'prediction', 'prediction_config_hash': digest(prediction_configuration())},
                    ...
                    if not results or results[-1].get('is_error') is not False or results[-1].get('terminal_reason') != 'completed':
                        raise GateError('PREDICTION_AGENT_INCOMPLETE')
                    if any(r.get('error') for r in gateway.records): raise GateError('PREDICTION_PROVIDER_FAILED')
                    if not any(r.get('model_responses') == [MODEL] for r in gateway.records):
                        raise GateError('PREDICTION_PROVIDER_UNVERIFIED')
                with core.store.tx() as db:
                    run = tools.validate(db)
                    result = self._receipt(db, run, agent_job_id)
                    core._event(db, run_id, 'prediction_agent_completed', job_id=agent_job_id,
                                executor=PREDICTION_AGENT_EXECUTOR.identity, configuration_hash=digest(prediction_configuration()))
                    db.execute("UPDATE jobs SET status='ACCEPTED' WHERE id=?", (agent_job_id,))
                return result
            except Exception as exc:
                code = exc.code if isinstance(exc, GateError) else 'PREDICTION_AGENT_FAILED'
