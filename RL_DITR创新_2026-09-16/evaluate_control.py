...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--config',required=True);ap.add_argument('--checkpoint');ap.add_argument('--mode',choices=['actor','beam','chunk','nominal'],default='beam');ap.add_argument('--horizon',type=int,default=48);ap.add_argument('--block-steps',type=int,default=16);ap.add_argument('--name',required=True);ap.add_argument('--batch-size',type=int,default=16);args=ap.parse_args()
 ...
 manifest={'config':cfg,'mode':args.mode,'checkpoint':args.checkpoint,'checkpoint_sha256':hashlib.sha256(Path(args.checkpoint).read_bytes()).hexdigest() if args.checkpoint else None,'source_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in ROOT.glob('*.py')},'jobs':jobs,'comparison_status':cfg['comparison_status'],'baseline_reruns':False,'seed_is_scenario_not_training':True}
 ...
 (out/'manifest.json').write_text(json.dumps(manifest,indent=2,allow_nan=False));server=None;all_results=[];all_latencies=[];start=time.time()
 if args.mode!='nominal':
  if not args.checkpoint:raise ValueError('Checkpoint required')
  command=[str(ROOT.parent/'.venv-native/bin/python'),str(ROOT/'policy_worker.py'),'--checkpoint',str(Path(args.checkpoint).resolve()),'--mode',args.mode]
