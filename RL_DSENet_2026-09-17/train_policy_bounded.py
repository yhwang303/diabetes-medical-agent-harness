...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--world',default='D02_reference_response');ap.add_argument('--name',default='D04_bounded_policy');args=ap.parse_args()
    world_folder=ROOT/'results'/args.world
    assert json.loads((world_folder/'development_diagnostics.json').read_text())['decision_gate_passed']
    assert json.loads((world_folder/'policy_mechanics.json').read_text())['status']=='passed'
    assert json.loads((ROOT/'checks/bounded_policy_mechanics.json').read_text())['status']=='passed'
    ...
    config={'seed':260915,'epochs':1,'batch_size':256,'lr':3e-4,'kl_beta':.05,
        ...
        'candidates':'fixed observed anchor, or +/-0.25U/h in one of three80min blocks;7 plans,48x5min',
        'anchor':'earliest observed history basal within0.25 of latest actual basal for training; at control start freeze latest observed warmup basal for entire episode',
        'reason':'development E02/E03 recurrent re-centering escaped paired intervention coverage; explicit persistent candidate reference, not medical safety gate',
        ...
        'world_checkpoint':str((world_folder/'world.pt').relative_to(ROOT)),
    ...
    config['source_sha256']={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (ROOT/'train_policy_bounded.py',ROOT/'policy_bounded.py',ROOT/'world_model.py')}
    ...
    with (out/'history.jsonl').open('w') as log:
        for batch in data.batches(256,260915,horizon=48):
            ...
            with torch.no_grad():
                z,f=patient.encode(x);rate=patient.reference_rate(x);anchor=history_anchor(x);actions=candidate_actions(rate,anchor)
                reference=patient.reference(z,f,rate);q=patient.utility(patient.trajectories(z,f,rate,actions))
                assert torch.isfinite(q).all()
            logits=policy(z,reference,rate,anchor);logp=logits.log_softmax(-1);p=logp.exp();advantage=q-q[:,0:1]
