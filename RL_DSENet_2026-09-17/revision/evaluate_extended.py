...
BASE=ROOT.parent
PROJECT=BASE.parent
sys.path.insert(0,str(PROJECT/'RL进阶对比_2026-09-15'))
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--common-limit',type=float,default=None);ap.add_argument('--bounded-reference',action='store_true');ap.add_argument('--config',required=True);ap.add_argument('--checkpoint');ap.add_argument('--mode',choices=['actor','beam','planner','hold','nominal','legacy','ditr','external'],default='beam');ap.add_argument('--horizon',type=int,default=48);ap.add_argument('--block-steps',type=int,default=16);ap.add_argument('--name',required=True);ap.add_argument('--batch-size',type=int,default=16);args=ap.parse_args()
 ...
 manifest={'common_limit_u_h':args.common_limit,'scorer_unchanged':True,'bounded_reference':args.bounded_reference,'config':cfg,'mode':args.mode,'checkpoint':args.checkpoint,'checkpoint_sha256':hashlib.sha256(Path(args.checkpoint).read_bytes()).hexdigest() if args.checkpoint else None,'source_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in ROOT.glob('*.py')},'jobs':jobs,'comparison_status':cfg['comparison_status'],'baseline_retraining':False,'frozen_baseline_reevaluation':args.mode in ('legacy','ditr'),'seed_is_scenario_not_training':True}
 ...
 (out/'manifest.json').write_text(json.dumps(manifest,indent=2,allow_nan=False));server=None;all_results=[];all_latencies=[];start=time.time();hold_rates={};projection_records=[]
 if args.mode not in ('nominal','hold'):
  if not args.checkpoint:raise ValueError('Checkpoint required')
  command=[str(PROJECT/'.venv-native/bin/python'),str(BASE/('legacy_policy_worker.py' if args.mode=='legacy' else ('bounded_policy_worker.py' if args.bounded_reference else 'new_policy_worker.py'))),'--checkpoint',str(Path(args.checkpoint).resolve()),'--mode',args.mode]
  if args.mode=='external':command=[str(PROJECT/'.venv-native/bin/python'),str(ROOT/'external_worker.py'),'--checkpoint',str(Path(args.checkpoint).resolve())]
  if args.mode=='ditr':command=[str(PROJECT/'.venv-native/bin/python'),str(PROJECT/'RL_DITR创新_2026-09-16/policy_worker.py'),'--checkpoint',str(Path(args.checkpoint).resolve()),'--mode','beam']
 ...
 try:
  for offset in range(0,len(jobs),args.batch_size):
   ...
   while active:
    ...
    if args.mode=='nominal':actions=[n for _,_,_,n in pending]
     ...
     if args.mode in ('legacy','external'):
    ...
    if args.common_limit is not None:
     projected=[]
  ...
  (out/'action_projection.json').write_text(json.dumps(projection_records,allow_nan=False))
